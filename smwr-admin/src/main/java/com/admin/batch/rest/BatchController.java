...
public class BatchController {
	...
	public ResponseEntity<?> runBatch(@RequestBody Map<String, Object> param) {
		...
		BatchConfig batchConfig = (BatchConfig) applicationContext.getBean("batchConfig");
		BatchConfig.RunOnceResult runResult = batchConfig.runOnce(jobKey, jobData);

		Map<String, Object> result = new HashMap<>();
		result.put("result", runResult.success ? Constant.SUCCESS : Constant.FAIL);
		if (runResult.message != null) {
			result.put("message", runResult.message);
		}
		return new ResponseEntity<>(result, runResult.success ? HttpStatus.OK : HttpStatus.BAD_REQUEST);
	}
