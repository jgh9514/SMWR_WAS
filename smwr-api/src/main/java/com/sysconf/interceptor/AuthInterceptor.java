...
import com.smw.common.constants.Constant;
...
public class AuthInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		// SessionInterceptor가 이미 주입한 경우(대부분) 재사용하여 중복 토큰/DB 조회 방지
		Object attr = request.getAttribute("userInfo");
		if (attr instanceof Map) {
			@SuppressWarnings("unchecked")
			Map<String, Object> existing = (Map<String, Object>) attr;
			if (existing.get("sess_user_id") != null) {
				cookieUtil.extendToken(request, response, Constant.LOGIN_TOKEN_NAME);
				return true;
			}
		}
		
		// fallback: 세션 주입이 없으면(예외 케이스) 토큰 파싱
